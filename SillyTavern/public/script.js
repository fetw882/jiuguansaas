...
async function importCharactersTags(avatarFileNames) {
    ...
    for (let i = 0; i < avatarFileNames.length; i++) {
        if (power_user.tag_import_setting !== tag_import_setting.NONE) {
            const avatarName = String(avatarFileNames[i] || '');
            const importedCharacter = characters.find(character => {
                const av = String(character?.avatar || '');
                return av === avatarName || av.endsWith('/' + avatarName) || av.endsWith('\\' + avatarName) || av.includes(avatarName);
            });
            if (!importedCharacter) {
                console.warn('importCharactersTags: character not found for', avatarName);
                continue;
            }
            if (!Array.isArray(importedCharacter.tags)) {
                importedCharacter.tags = Array.isArray(importedCharacter.data?.tags) ? importedCharacter.data.tags : [];
            }
