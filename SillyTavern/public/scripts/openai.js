...
export async function prepareOpenAIMessages({
    ...
    // 防御：部分第三方源（如 Gemini/Makersuite）在本地计数可能与实际不一致，
    // 这里放宽预算，避免“Mandatory prompts exceed the context size”提前拦截。
    let ctxBudget = userSettings.openai_max_context;
    let rspBudget = userSettings.openai_max_tokens;
    try {
        if (oai_settings.chat_completion_source === chat_completion_sources.MAKERSUITE || /^(gemini)/i.test(getChatCompletionModel())) {
            ctxBudget = Number.MAX_SAFE_INTEGER;
            rspBudget = 0;
        }
    } catch {}
    chatCompletion.setTokenBudget(ctxBudget, rspBudget);
...
export class ChatCompletion {
    ...
    checkTokenBudget(message, identifier) {
        try {
            // 放宽：对 Makersuite/Gemini 源跳过本地预算拦截，避免未发请求即报错
            if (oai_settings?.chat_completion_source === chat_completion_sources.MAKERSUITE || /^gemini/i.test(getChatCompletionModel())) {
                return;
            }
        } catch {}
