...
async function autoBackgroundCommand() {
    ...
    // 防御：空回复或非字符串时，避免 Fuse 试图解析逻辑表达式报错
    if (typeof reply !== 'string') {
        reply = String(reply?.text || reply || '');
    }
    const fuse = new Fuse(options, { keys: ['text'] });
    let bestMatch = [];
    try {
        bestMatch = fuse.search(reply || '', { limit: 1 });
    } catch (e) {
        console.warn('Auto background: Fuse search failed, falling back.', e);
        bestMatch = [];
    }
