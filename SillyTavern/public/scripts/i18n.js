...
async function getLocaleData(language) {
    ...
    // try relative under current base first, then absolute /locales fallback
    const tryPaths = [`./locales/${language}.json`, `/locales/${language}.json`, `${location.origin}/locales/${language}.json`];
    for (const url of tryPaths) {
        try {
            const response = await fetch(url);
            console.log(`Loading locale data from ${url}`);
...
export async function initLocales() {
    // Load languages list with robust fallbacks
    async function loadLangsList() {
        const urls = [
            '/locales/lang.json',
            './locales/lang.json',
            `${location.origin}/locales/lang.json`,
        ];
    ...
    try {
        langs = await loadLangsList();
    } catch {
        langs = [{ lang: 'en', display: 'English' }];
    }
